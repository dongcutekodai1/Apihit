...
# --------------------------
# Thuật toán dự đoán nâng cao (port từ JS)
def ai_predict(history):
    ...
    if abs(tai_count - xiu_count) / len(recent) >= 0.25:
        if tai_count > xiu_count: xiu_score += 0.25
        else: tai_score += 0.25
    ...
    # Streak logic
    if streak >= 6:
        if last == "Tài": xiu_score += 0.35
        else: tai_score += 0.35
    elif streak >= 4:
        if last == "Tài": tai_score += 0.15
        else: xiu_score += 0.15
...
def update_result(store, history, lock, result):
    with lock:
        result["Du_doan"] = ai_predict(history)
...
# --------------------------
# Poll dữ liệu từ API gốc (MD5 only)
def poll_md5():
    global last_sid_md5
...
def get_history():
    with lock_md5:
        return jsonify({"taixiumd5": history_md5})
